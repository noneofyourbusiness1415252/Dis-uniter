const { createServer, get } = require("http"),
  ...
  { fork, isPrimary, worker } = require("cluster"),
  { env } = process;
/** @param {Client} bot - Client object to run */
module.exports = (bot) => {
  ...
  bot.on("ready", async () => {
    ...
    if (isPrimary) {
      for (_ in require("os").cpus()) fork();
      console.log(`${bot.user.tag} alive!`);
    } else {
      createServer((_, res) => {
        ...
        res.end(
          `<!DOCTYPE html><meta charset=utf-8><meta name=viewport content='width=device-width'><meta name=description content='${
            ...
            bot.readyTimestamp
          })/1000));document.getElementById('u').innerText=\`\${u>86400n?\`\${u/86400n}d\`:''}\${u>3600n?\`\${u/3600n%60n}h\`:''}\${u>60n?\`\${u/60n%24n}m\`:''}\${\`\${u%60n}\`}s\`}, 1000)}</script><style>*{background-color:#FDF6E3;color:#657B83;text-align:center;margin:auto;font-family:sans-serif}@media(prefers-color-scheme:dark){*{background-color:#002B36;color:#839496}}img{height:1em}td{border:1px}</style><html lang=en><html lang=en><h1>${
            user.tag
            ...
            owner.banner
          } alt></table><small>Provided by <a href=https://www.npmjs.com/package/replit-dis-uniter>replit-dis-uniter`
        );
      }).listen(80);
    }
    if (isPrimary) {
      get(
        `http://up.repl.link/add?repl=${env.REPL_SLUG}&author=${env.REPL_OWNER}`
      );
      console.log(
        "Press enter at any time to update user and application info to show on the website"
      );
    }
